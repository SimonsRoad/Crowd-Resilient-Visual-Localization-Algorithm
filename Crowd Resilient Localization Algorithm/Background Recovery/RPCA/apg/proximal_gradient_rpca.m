...
while ~converged
    ...
    Y_k_A = X_k_A + ((t_km1 - 1)/t_k)*(X_k_A-X_km1_A) ;   % Algotihm1 step 4
    Y_k_E = X_k_E + ((t_km1 - 1)/t_k)*(X_k_E-X_km1_E) ;   % Algotihm1 step 4
    
    if ~lineSearchFlag
        
        G_k_A = Y_k_A - (1/tau_k)*(Y_k_A+Y_k_E-D) ;  % Algotihm1 step 5
        G_k_E = Y_k_E - (1/tau_k)*(Y_k_A+Y_k_E-D) ;  % Algotihm1 step 7
        
        [U,S,V] = svd(G_k_A,'econ') ;  % Algotihm1 step 6
        diagS = diag(S) ; 
        
        X_kp1_A = U * diag(pos(diagS-mu_k/tau_k)) * V';   % Algotihm1 step 6
        X_kp1_E = sign(G_k_E) .* pos( abs(G_k_E) - lambda*mu_k/tau_k );  % Algotihm1 step8
    ...
    t_kp1 = 0.5*(1+sqrt(1+4*t_k*t_k)) ;  % Algotihm1 step 9
    % Start from line 209 to line 214 is different than the parameters and
    % equations listed in the paper. 
    % Note: my strategy is to try with this version first, then change this
    % part following paper and try it again.
